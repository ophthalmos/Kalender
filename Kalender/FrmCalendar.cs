...
namespace Kalender
{
    public partial class FrmCalendar : Form
    {
        ...
        private int currentYear = DateTime.Now.Year; // Wert wird nicht geändert
        ...
        private readonly Config config = new Config(Path.Combine(Application.StartupPath, appName + ".xml"));
        ...
        public FrmCalendar(string[] arguments)
        {
            ...
            config.Sections[appName].Settings.Add(defGoogleCalendar, varGoogleCalendar);
            ...
            try
            {
                ...
                varGoogleCalendar = (GetConfigValue(defGoogleCalendar));
        ...
        {// blInfo.BackColor = monthCalendar.SelectionRange.Start != monthCalendar.SelectionRange.End ? SystemColors.ActiveCaption : SystemColors.ControlLightLight;
            ...
            lblInfo.Text = monthCalendar.SelectionRange.Start != monthCalendar.SelectionRange.End ? (lblInfo.Text.Length > 0 ? "Spanne: " + lblInfo.Text : "") : lblInfo.Text;
        ...
        private void FrmCalendar_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            else
            {
                ...
                SaveConfigValue(defGoogleCalendar, varGoogleCalendar);
        ...
        {
            using (FrmSettings frmSettings = new FrmSettings(varColorScheme, varAlwaysOnTop, varTrayModus, varHotkeyLetter, varGoogleCalendar, varTooltipText))
            {
                ...
                if (frmSettings.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (!foo.Equals(varGoogleCalendar) || !bar.Equals(varTooltipText))
                    {
                        varGoogleCalendar = foo;
                        varTooltipText = bar;
                        if (MessageBox.Show("Die Einstellungen bezüglich des externen Google-\nKalenders werden erst bei einem Neustart wirksam.\n\nMöchten Sie dieses Programm jetzt neustarten?", appName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            Reload(true);
                        }
                    }
